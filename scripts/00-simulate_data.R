...
peaks <- data.frame(
  ...
  peak_alt = c("Sagarmatha","Chhogori",NA,"E1","Kamalung",NA,NA,NA,NA,"Hidden Peak",NA,NA,"Gosainthan","Diamir",NA,NA,NA,NA,NA,"Everest's Daughter"),
  peak_height = (5000 + 4000 * rbeta(n_peaks,2,5)),
  climbing_status = sample(c("Climbed", "Unclimbed"),n_peaks,replace = TRUE, prob = c(0.8,0.2)),
  first_ascent_year = sample(1905:2019, n_peaks, replace = TRUE),
  first_ascent_country = sample(c("Nepal", "India", "China", "USA", "UK"), n_peaks, replace = TRUE, prob = c(0.4,0.2,0.2,0.1,0.1)),
...
expeditions <- data.frame(
  ...
  season = sample(c("Spring", "Summer", "Autumn", "Winter"), n_expeditions, replace = TRUE,prob = seasons_prob),
...
members <- data.frame(
  ...
  peak_name = peaks$peak_name[match(sample(peaks$peak_id, n_members, replace = TRUE), peaks$peak_id)],
  year = sample(1905:2019, n_members, replace = TRUE),
  season = sample(c("Spring", "Summer", "Autumn", "Winter"), n_members, replace = TRUE),
  sex = sample(c("Male", "Female"), n_members, replace = TRUE),
  age = sample(18:70, n_members, replace = TRUE),
  citizenship = sample(c("Nepal", "India", "USA", "UK", "China"), n_members, replace = TRUE),
  expedition_role = sample(c("Leader", "Climber", "Sherpa", "Support"), n_members, replace = TRUE),
  ...
  death_cause = sample(c("Avalanche", "Fall", "Altitude Sickness", "Other", NA), n_members, replace = TRUE),
  death_height_metres = ifelse(runif(n_members) < 0.1, sample(5000:9000, n_members, replace = TRUE), NA),
  injured = sample(c(TRUE, FALSE), n_members, replace = TRUE),
  injury_type = sample(c("Fracture", "Frostbite", "Altitude Sickness", "Other", NA), n_members, replace = TRUE),
  injury_height_metres = ifelse(runif(n_members) < 0.1, sample(5000:9000, n_members, replace = TRUE), NA)
)
#### Save data ####
