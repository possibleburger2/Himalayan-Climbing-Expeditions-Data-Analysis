...
The Himalayan mountain range spans X KM (source), which means that the graphed distribution does not represent the distribution of ALL peaks in the Himalayas. Instead this distribution represents the distribution of peaks commonly attempted/climbed.The Himalayas starting at X M starting above sea level combined with the lowest height required to be considered a peak, the distribution starts at 5000m above sea level.
...
@fig-trend Expectly most first ascents are from the country that the Himalayas reside in, followed by Japan, UK, France, Germany, then the USA..
\newpage
...
shows us that despite removing citizenship and hired_staff the RMSE only grains 0.01, meaing it loses 1% accuracy. This means the citizenship of climber nor the amount of hired staff did not contribute to the model's accuracy and were good to remove.
